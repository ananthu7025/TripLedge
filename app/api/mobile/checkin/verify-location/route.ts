...
import { db } from '@/db';
import { checkinRequests, attendance, wifiConfigs } from '@/db/schema';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Attempt to get office location from DB
        const officeConfig = await db.query.companySettings.findFirst();

        const officeLat = officeConfig?.officeLatitude ? parseFloat(officeConfig.officeLatitude) : DEFAULT_OFFICE_LAT;
        const officeLng = officeConfig?.officeLongitude ? parseFloat(officeConfig.officeLongitude) : DEFAULT_OFFICE_LNG;

        const distance = getDistance(latitude, longitude, officeLat, officeLng);
        const isWithinRadius = distance <= RADIUS_METERS;

        if (isWithinRadius) {
            // Auto-approve and create attendance record
            ...
            const [newRequest] = await db.insert(checkinRequests).values({
                requestId,
                userId: user.id,
